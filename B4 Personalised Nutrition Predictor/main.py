...
global df, sc, train_or_load_dnn, dnn_model
global le, labels
...
def preprocess():
    ...
    X = df.drop(columns=["Caloric Requirement (kcal/day)"])
    y = df["Caloric Requirement (kcal/day)"]
...
def RidgeRegressor():
    ...
    if os.path.exists(Regressor):
        ...
        ridge_reg = Ridge(alpha=0, solver='sparse_cg', max_iter=5, tol=1)
...
# Main window setup
main = Tk()
main.title("Personalized Nutrition Prediction")
...
title = Label(main, text='Personalized Nutrition Predictor Employing Deep Learning Models to Estimate Individual Nutritional Needs Based on Health Data',
              bg='#282828', fg='#FFD700', font=font, height=2)  # Dark background with Gold text
title.pack(fill=X, pady=10)

# Frame to hold buttons and text console
main_frame = Frame(main, bg='azure')  # Navy Blue for consistency
main_frame.pack(fill=BOTH, expand=True, padx=20, pady=20)

# Frame to hold buttons (centered and in two rows)
button_frame = Frame(main_frame, bg='azure')  # Consistent background
